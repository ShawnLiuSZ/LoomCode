...
func TestEffortManager(t *testing.T) {
	...
	t.Run("GetMaxSteps", func(t *testing.T) {
		...
		if manager.GetMaxSteps() != 5 {
			t.Errorf("expected 5 steps for low, got %d", manager.GetMaxSteps())
		}

		manager.SetLevel(EffortMedium)
		if manager.GetMaxSteps() != 10 {
			t.Errorf("expected 10 steps for medium, got %d", manager.GetMaxSteps())
		}

		manager.SetLevel(EffortHigh)
		if manager.GetMaxSteps() != 20 {
			t.Errorf("expected 20 steps for high, got %d", manager.GetMaxSteps())
		}
	})
