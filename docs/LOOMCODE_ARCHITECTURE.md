...
> 日期: 2026-06-18  
> 形态: 纯 CLI 工具（不做桌面应用）  
> 基于: [LOOMCODE_PLAN.md](./LOOMCODE_PLAN.md)
...
# 步骤3: 使用
loomcode --provider qwen --model qwen-max
```
...
| 全局记忆 | `~/.loomcode/memory.db` | 用户偏好、跨项目规则 |
| SQLite 历史 | `~/.loomcode/sessions.db` | FTS5 全文搜索原始对话 |
